...
class ReliableSenderNode(Router):
    ...
    def reliable_send(self, time):
        ## Your code here
        if self.lpk == None:
            self.lpk = self.send_pkt(time, self.counter, time, 'green')
            self.counter = self.counter + 1
        elif time - self.lpk.start > self.timeout:
            self.lpk = self.send_pkt(time, self.lpk.properties['seqnum'], time, 'yellow')
        return self.lpk
    ...
    def process_ack(self, time, acknum, timestamp):
        ## Your code here
        if self.lpk == None or acknum > self.lpk.properties['seqnum']:
            print "something wrong with ACK\n"
        elif acknum == self.lpk.properties['seqnum']:
            self.lpk = None
...
class ReliableReceiverNode(Router):
    ...
    def reliable_recv(self, sender, time, seqnum, pkt_timestamp):
        ...
        if self.app_seqnum + 1 == seqnum:
            ...
            print "something wrong with recv_seqnum\n"
        return
