...
class ReliableSenderNode(Router):
    ...
    def reliable_send(self, time):
        ## Your code here
        for i in range(len(self.sendbuf)):
            if time - self.sendbuf[i].start > self.timeout:
                self.sendbuf[i] = self.send_pkt(time, self.sendbuf[i].properties['seqnum'], time, 'yellow')
        while len(self.sendbuf) < self.window:
            self.sendbuf.append(self.send_pkt(time, self.counter, time, 'green'))
    ...
    def process_ack(self, time, acknum, pkt_timestamp):
        ## Your code here
        for p in self.sendbuf:
            if p.properties['seqnum'] == acknum:
                break
        self.sendbuf.remove(p)
    ...
    def calc_timeout(self, time, pkt_timestamp):
        ...
        self.srtt = 0.125 * (time - pkt_timestamp) + self.srtt * 0.875
        self.rttdev = 0.25 * abs(time - pkt_timestamp - self.srtt) \
                      + 0.75 * abs(self.rttdev)
        timeout = self.srtt + 4 * self.rttdev
        return timeout
...
class ReliableReceiverNode(Router):
    ...
    def reliable_recv(self, sender, time, seqnum, pkt_timestamp):
        ...
        if self.app_seqnum + 1 == seqnum:
            self.app_receive(seqnum, time)
            status = True
            while status:
                leng = len(self.rcvbuf)
                for i in range(leng):
                    if self.rcvbuf[i][0] == self.app_seqnum + 1:
                        self.app_receive(self.rcvbuf[i][0], self.rcvbuf[i][1])
                        self.rcvbuf.pop(i)
                        break
                if leng == len(self.rcvbuf):
                    status = False       
        elif self.app_seqnum + 1 < seqnum:
            self.rcvbuf.append((seqnum, time))
        return
