...
class LSRouter(Router):
    ...
    def make_ls_advertisement(self):
        ...
        for i in self.neighbors:
            neighbors_info.append((self.neighbors[i][1], self.neighbors[i][2]))
    ...
    def run_dijkstra(self, nodes):
        ...
        #init neighbors of current node
        toprocess = []
        for n in self.neighbors:
            self.routes[self.neighbors[n][1]] = n
            self.spcost[self.neighbors[n][1]] = self.neighbors[n][2]
            toprocess.append(self.neighbors[n][1])
            nodes.remove(self.neighbors[n][1])
        #init other nodes
        for n in nodes:
            self.routes[n] = None
            self.spcost[n] = self.INFINITY
            toprocess.append(n)
        #process routes and spcost
        while len(toprocess) > 0:
            close = None
            distance = self.INFINITY
            #find the closest node
            for k in toprocess:
                if self.spcost[k] < distance:
                    close = k
                    distance = self.spcost[k]
                    
            if close == None:
                print "Do not find closest node\n"
            #updat its neighbors
            elif self.LSA.get(close) != None:
                ngr_info = self.LSA[close][1:]
                for ngr in ngr_info:
                    if self.spcost[ngr[0]] > distance + ngr[1]:
                        self.spcost[ngr[0]] = distance + ngr[1]
                        self.routes[ngr[0]] = self.routes[close]
            toprocess.remove(close)
