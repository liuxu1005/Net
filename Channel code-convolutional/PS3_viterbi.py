...
class ViterbiDecoder:
    ...
    def viterbi_step(self,n,received_voltages):
        ...
        for i in range(self.nstates):
            bm1 = self.PM[self.predecessor_states[i][0], n-1] +\
                  self.branch_metric(self.expected_parity[self.predecessor_states[i][0]][i],received_voltages)
            bm2 = self.PM[self.predecessor_states[i][1], n-1] +\
                  self.branch_metric(self.expected_parity[self.predecessor_states[i][1]][i],received_voltages)
            if bm1 >= bm2:
                self.PM[i,n] = bm2
                self.Predecessor[i, n] = self.predecessor_states[i][1]
            else:
                self.PM[i,n] = bm1
                self.Predecessor[i, n] = self.predecessor_states[i][0]
        return
    ...
    def traceback(self,s,n):
        #your code here
        if n < self.K:
            rlt =  [int(i) for i in bin(s)[2: n + 2]]
            missing = n - len(rlt)
            while missing > 0:
                rlt.insert(0, 0)
                missing = missing - 1
        ...
        rlt = [int(i) for i in bin(s)[2:]]
        missing = self.K - 1 - len(rlt)
        while missing > 0 :
            rlt.insert(0,0)
            missing = missing - 1
      
        while n >= self.K :
            rlt.append(int(bin(self.Predecessor[s, n])[-1]))
